...
class GooglePhotosClient:
    ...
    def _list_media_items(self, page_size: int = 100, page_token: Optional[str] = None):
        ...
        return self.service.mediaItems().list(
            pageSize=page_size,
    ...
    def get_photos(self, include_albums: bool = False) -> List[Dict]:
        ...
        while True:
            try:
                ...
                photos.extend([item for item in items if item['mimeType'].startswith('image/')])
        ...
        while True:
            try:
                albums = self.service.albums().list(
                    pageSize=50,
                    pageToken=next_album_page_token
                ).execute()
                ...
                for album in albums.get('albums', []):
                    ...
                    while True:
                        ...
                        album_photos.update(item['id'] for item in search_results.get('mediaItems', []))
